...
def _fmt(v, prefix="R$"):
    ...
    s = "-" if v < 0 else ""
    a = abs(v)
    if a >= 1e9: return f"{s}{prefix} {a/1e9:,.2f} bi"
    if a >= 1e6: return f"{s}{prefix} {a/1e6:,.1f} mi"
...
def _card(label, value, vc="#f8fafc", badge_text="", badge_style="", tip=""):
    ...
    bdg = (f'<div style="font-size:0.65rem;font-weight:700;font-family:monospace;'
...
def _green_badge(txt):
    return txt, "color:#00ff87;background:rgba(0,255,135,0.08);border:1px solid rgba(0,255,135,0.2)"

def _red_badge(txt):
    return txt, "color:#ff3d5a;background:rgba(255,61,90,0.08);border:1px solid rgba(255,61,90,0.2)"
    ...
    try:
        ...
        for i in range(n):
            ...
            cash  = _v(bs,  ['Cash And Cash Equivalents',
                              'Cash Cash Equivalents And Short Term Investments'], i)
            dbt_s = _v(bs,  ['Current Debt', 'Current Portion Of Long Term Debt'], i) or 0.0
            dbt_l = _v(bs,  ['Long Term Debt',
                              'Long Term Debt And Capital Lease Obligation'], i) or 0.0
            equity = _v(bs, ['Stockholders Equity',
                              'Total Equity Gross Minority Interest'], i)

            if ebit is None or rev is None:
                continue

            t_rate  = min(max(abs(taxex)/abs(pretx), 0.10), 0.40) if pretx and taxex and abs(pretx) > 0 else 0.34
            ...
            years.append({
                'year': ylbl, 'revenue': rev, 'ebit': ebit, 'tax_rate': t_rate,
                'noplat': noplat, 'da': da_v, 'capex': cap_v, 'delta_wc': dwc_v, 'fcf': fcf,
                'wco': wco, 'ppe': ppe or 0, 'goodwill': gw, 'ic': ic,
                'debt': dbt_s + dbt_l, 'equity': equity or 0, 'cash': cash or 0,
                'interest': intr,
            })
        ...
        for i, y in enumerate(ys):
            ...
            y['roic'] = (y['noplat'] / ic_prev * 100) if (ic_prev and ic_prev > 1e4) else None
            y['roic_no_gw'] = (y['noplat'] / (ic_prev - ys[i-1]['goodwill']) * 100
                               if (ic_prev and ic_prev - ys[i-1]['goodwill'] > 1e4) else None)

        shares = (info.get('sharesOutstanding') or
                  info.get('impliedSharesOutstanding') or 1)
        beta   = float(info.get('beta') or 1.0)
        price  = float(info.get('currentPrice') or
                       info.get('regularMarketPrice') or 0)
        lat    = ys[-1]
        kd_est = (abs(lat['interest']) / lat['debt'] * 100
                  if lat.get('interest') and lat['debt'] > 0 else None)
        ...
        return {
            'years': ys, 'latest': lat,
            'shares': shares, 'beta': beta, 'price': price,
            'total_debt': lat['debt'], 'cash': lat['cash'],
...
def calc_dcf(noplat0, g1_pct, g2_pct, gt_pct, wacc_dec, roic_cv_pct):
    ...
    for t in range(1, 11):
        ...
        pv_exp += pv_t
        rows.append({'t': t, 'g_pct': g_t*100, 'noplat': noplat_t,
                     'reinv_pct': reinv*100, 'fcf': fcf_t, 'pv': pv_t})
...
with col_t:
    ...
    ticker = st.text_input(
        "Ticker B3", value=defaults[0] if defaults else "",
...
if not ticker:
    with col_hint:
        st.info("Digite um ticker B3 para iniciar o valuation completo (metodologia Koller).")
    st.stop()
...
if skey not in st.session_state:
    ...
    st.session_state[skey] = {
        'ke': _ke, 'kd': _kd, 'e_weight': _ew,
        'g1': _g1, 'g2': round(_g1 * 0.5, 1), 'gt': 3.5,
...
wacc_dec  = (ss['ke']/100 * ss['e_weight']
             + ss['kd']/100 * (1 - ss['e_weight']) * (1 - tax_rate))
...
with tabs[0]:
    ...
    model_data = [
        ("Enterprise DCF com WACC", "Estrutura de capital estável (D/V constante)",
         "#00ff87", "✅ Este modelo"),
        ("APV — Adjusted Present Value", "Estrutura de capital muda significativamente",
         "#00d2ff", "Alternativo"),
        ("Economic Profit", "Foco em destruição/criação de valor por período",
         "#ffd600", "Check interpretativo"),
        ("Equity Cash Flow", "Banco, seguradora ou financeira",
         "#f87171", "Para financeiras"),
        ("Múltiplos de Mercado", "Checagem rápida ou sem projeções detalhadas",
         "#94a3b8", "Validação"),
    ]

    for name, when, color, badge_txt in model_data:
        is_sel = name.startswith("Enterprise")
        border = f"border-left:3px solid {color}" if is_sel else f"border-left:3px solid #1e293b"
    ...
    st.info("**Este modelo usa Enterprise DCF** como primary + Economic Profit como check interpretativo. "
    ...
    if len(ys) < 2:
        ...
        with col_r:
            ...
            fig_roic.add_trace(go.Bar(
                x=years_lbl, y=roic_vals,
                marker_color=['#00ff87' if (r and r > wacc_pct) else '#ff3d5a'
                              for r in roic_vals],
                name='ROIC (%)',
                text=[f"{r:.1f}%" if r else "—" for r in roic_vals],
                textposition='outside',
            ))
            fig_roic.add_hline(y=wacc_pct, line_dash="dash",
                               line_color="#ffd600",
                               annotation_text=f"WACC {wacc_pct:.1f}%",
                               annotation_position="bottom right")
            fig_roic.update_layout(title="ROIC histórico vs WACC", height=280,
                                   yaxis_title="ROIC (%)")
        ...
        with col_f:
            ...
            fig_fcf = go.Figure(go.Bar(
                x=years_lbl, y=fcf_vals,
                marker_color=['#00ff87' if v >= 0 else '#ff3d5a' for v in fcf_vals],
                text=[f"R$ {v:,.0f}M" for v in fcf_vals],
                textposition='outside',
            ))
            fig_fcf.update_layout(title="Free Cash Flow histórico (R$ mi)", height=280,
...
with tabs[2]:
    ...
    if roic_med is not None:
        ...
        spread_lbl, spread_style = (
            ...
            if spread > 0 else
            _red_badge(f"ROIC < WACC: {spread:.1f}pp — DESTRÓI VALOR")
        )
        spread_cards_html += _card("ROIC Mediano", _pct(roic_med),
                                   "#00ff87" if roic_med > wacc_pct else "#ff3d5a",
                                   badge_text=spread_lbl, badge_style=spread_style)
    spread_cards_html += _card("WACC Estimado", _pct(wacc_pct), "#ffd600")
    spread_cards_html += _card("CAGR Receita", _pct(cagr), "#00d2ff",
                               tip="Taxa de crescimento anual composta da receita histórica")
    st.markdown(f'<div class="mcard-grid">{spread_cards_html}</div>', unsafe_allow_html=True)
    ...
    benchmarks = [
        ...
        ("ROIC de alto desempenho", "Declina para ~15% em 15 anos",
        ...
        ("Fortune 50", "~1% real após ingresso no ranking",
    ...
    st.warning("**Regra de ouro:** Seja cético com crescimento > 10% por mais de 5 anos, "
    ...
    if roic_med is not None and latest['ic'] > 0:
        ...
        ep_cards = (
            _card("Economic Profit", _fmt(ep),
                  "#00ff87" if ep > 0 else "#ff3d5a",
                  tip="EP = (ROIC − WACC) × IC. Positivo = cria valor; negativo = destrói.") +
            (_card("EP / Ação", f"R$ {ep_ps:,.2f}", "#00ff87" if ep_ps and ep_ps > 0 else "#ff3d5a") if ep_ps else "")
        )
...
with tabs[3]:
    ...
    with col_sl1:
        g1 = st.slider("Crescimento Anos 1–5 (%)", 0.0, 30.0,
                       value=float(ss['g1']), step=0.5, key=f"{skey}_g1",
                       help=f"CAGR histórico da receita: {_pct(cagr)}. Seja conservador.")
        ss['g1'] = g1

    with col_sl2:
        g2 = st.slider("Crescimento Anos 6–10 (%)", 0.0, 20.0,
                       value=float(ss['g2']), step=0.5, key=f"{skey}_g2",
    ...
    if noplat0 <= 0:
        st.error("NOPLAT negativo — não é possível projetar o DCF. "
                 "Verifique se o EBIT é positivo nos dados históricos.")
    else:
        _, _, _, proj_rows, _, _ = calc_dcf(noplat0, g1, g2, ss['gt'], wacc_dec, roic_cv)
        ...
        fig_wf = go.Figure(go.Bar(
            ...
            marker_color=['#00ff87' if r['fcf'] >= 0 else '#ff3d5a' for r in proj_rows],
        ))
        fig_wf.update_layout(title="FCF Projetado (R$ mi)", height=240,
                              yaxis_title="FCF (R$ mi)")
        apply_plotly_theme(fig_wf)
        st.plotly_chart(fig_wf, use_container_width=True)

        st.caption(f"NOPLAT base: {_fmt(noplat0)} | "
...
""")
    ...
    with col_cv1:
        gt = st.slider("Crescimento Terminal — g (%)", 1.0, 6.0,
                       value=float(ss['gt']), step=0.25, key=f"{skey}_gt",
                       help="Taxa de crescimento na perpetuidade. PIB nominal BR ≈ 3–4%.")
        ss['gt'] = gt

    with col_cv2:
        roic_cv = st.slider("ROIC na Perpetuidade — ROIC_cv (%)", 5.0, 35.0,
                            value=float(ss['roic_cv']), step=0.5, key=f"{skey}_roic_cv",
                            help="Use WACC se não há moat sustentável; ROIC histórico se há vantagem competitiva.")
        ss['roic_cv'] = roic_cv

    if latest['noplat'] > 0:
        noplat0 = latest['noplat']
        noplat_11 = noplat0 * (1+ss['g1']/100)**5 * (1+ss['g2']/100)**5 * (1+gt/100)
        ...
        if cv is not None:
            ...
            cv_cards = (
                _card("NOPLAT T+11", _fmt(noplat_11), "#f8fafc",
                      tip="NOPLAT do primeiro ano após o horizonte explícito") +
                _card("Continuing Value", _fmt(cv), "#a855f7",
                      tip="CV não descontado — valor da perpetuidade") +
                _card("PV(CV)", _fmt(pv_cv), "#a855f7",
                      tip="Valor presente do Continuing Value") +
                _card("CV / EV Total", f"{cv_pct:.0f}%",
                      "#ffd600" if cv_pct > 80 else "#f8fafc",
                      tip="CV geralmente representa 60–80% do Enterprise Value. Acima de 80% requer atenção extra.")
            )
            st.markdown(f'<div class="mcard-grid">{cv_cards}</div>', unsafe_allow_html=True)

            if cv_pct > 80:
                st.warning(f"⚠️ CV representa **{cv_pct:.0f}%** do Enterprise Value — "
            ...
            if abs(roic_cv - wacc_dec*100) < 1:
                st.info("Quando ROIC_cv ≈ WACC, o crescimento não cria valor adicional — "
                        "use a fórmula simplificada: CV = NOPLAT_{T+11} / WACC.")
        else:
            st.error("WACC ≤ g terminal — modelo indefinido. Aumente o WACC ou reduza g.")
    else:
...
with tabs[5]:
    ...
    with col_ke:
        ...
        ke = st.slider("ke — Custo do Equity (%)", 6.0, 25.0,
                       value=float(ss['ke']), step=0.25, key=f"{skey}_ke",
    ...
    with col_kd:
        ...
        kd = st.slider("kd — Custo da Dívida (%)", 4.0, 20.0,
                       value=float(ss['kd']), step=0.25, key=f"{skey}_kd",
    ...
    e_weight = st.slider("Participação do Equity — E/(E+D) (%)", 20.0, 95.0,
                         value=float(ss['e_weight'] * 100), step=1.0,
                         key=f"{skey}_ew",
                         help="Use valor de mercado, não contábil. "
                              f"Estimado: {ss['e_weight']*100:.0f}%") / 100
    ...
    wacc_cards = (
        ...
        _card("WACC", _pct(wacc_pct), "#ffd600",
    ...
    fig_ww = go.Figure(go.Bar(
        ...
        text=[f"{v:.2f}%" for v in [ke*e_weight, kd*(1-tax_rate)*d_weight, wacc_pct]],
        textposition='outside',
    ))
    fig_ww.update_layout(title="Decomposição do WACC", height=260,
                         yaxis_title="(%)")
...
with tabs[6]:
    ...
    st.caption("Referência: Koller et al., Cap. 11 — *Calculating and Interpreting Results*")
    ...
    if noplat0 <= 0:
        ...
        st.error("WACC ≤ g terminal — modelo indefinido. Ajuste os parâmetros na aba WACC.")
    else:
        ...
        upside      = ((price_iv / price - 1) * 100) if price and price > 0 and price_iv else None
        ms          = (1 - price / price_iv) * 100 if price and price_iv and price_iv > 0 else None

        # Summary cards
        vc_up = "#00ff87" if (upside and upside > 0) else "#ff3d5a"
        verdict = ("SUBAVALIADO" if upside and upside > 15 else
                   "SOBREAVALIADO" if upside and upside < -15 else "PRÓXIMO DO JUSTO")
        vcolor  = "#00ff87" if verdict == "SUBAVALIADO" else ("#ff3d5a" if verdict == "SOBREAVALIADO" else "#ffd600")

        res_cards = (
            _card("PV (FCFs Explic.)", _fmt(pv_exp), "#00d2ff",
                  tip="Soma do valor presente dos FCFs projetados (anos 1–10)") +
            _card("PV (Continuing Value)", _fmt(pv_cv), "#a855f7",
                  tip="Valor presente do Continuing Value (perpetuidade)") +
            _card("Enterprise Value", _fmt(ev), "#ffd600",
                  tip="Enterprise Value = PV(FCFs) + PV(CV)") +
            _card("(−) Dívida Líquida", _fmt(-net_debt), "#f87171",
                  tip="Dívida total − Caixa. Ponte firma → ação.") +
            _card("Equity Value", _fmt(equity_val), "#00ff87",
        ...
        if price_iv:
            res_cards += (
                ...
                _card("Potencial", f"{upside:+.1f}%", vc_up,
                      badge_text=verdict, badge_style=f"color:{vcolor};background:{vcolor}18;border:1px solid {vcolor}44") +
                _card("Margem de Seg.", f"{ms:.1f}%" if ms else "—",
                      "#00ff87" if ms and ms > 20 else ("#ff3d5a" if ms and ms < 0 else "#ffd600"))
            )
        st.markdown(f'<div class="mcard-grid">{res_cards}</div>', unsafe_allow_html=True)
  ...
  <div style="display:flex;justify-content:space-between;margin-top:0.3rem">
    ...
    <span style="font-size:0.72rem;color:#a855f7">IV: R$ {price_iv:,.2f}</span>
        ...
        fig_bridge = go.Figure(go.Bar(
            ...
            text=[f"R$ {v/1e9:,.2f} bi\n({v/ev*100:.0f}%)" for v in [pv_exp, pv_cv, ev]],
            textposition='outside',
        ))
        fig_bridge.update_layout(title=f"Composição do Enterprise Value (CV = {cv_pct:.0f}% do EV)",
                                 height=280, yaxis_title="R$ bilhões")
        apply_plotly_theme(fig_bridge)
        st.plotly_chart(fig_bridge, use_container_width=True)

        # Sensitivity heatmap
        with st.expander("📊 Sensibilidade — WACC × g terminal (Preço por Ação)", expanded=True):
            st.caption(
            ...
            wacc_range = [w/100 for w in range(
                max(6, int(wacc_pct)-4), min(22, int(wacc_pct)+5))]
            ...
            for w in wacc_range:
                ...
                for g in gt_range:
                    if w <= g/100:
                        row_z.append(None); row_t.append("N/A"); continue
                    _, _, ev_s, _, _, _ = calc_dcf(
                        noplat0, ss['g1'], ss['g2'], g, w, ss['roic_cv'])
                    eq_s = ev_s - net_debt
                    piv_s = eq_s / shares if shares > 0 else None
                    up_s = ((piv_s / price - 1) * 100
                            if price and price > 0 and piv_s else None)
                    row_z.append(up_s)
            ...
            fig_s = go.Figure(go.Heatmap(
                ...
                colorscale="RdYlGn", zmid=0, zmin=-80, zmax=80,
                text=z_text,
                texttemplate="%{text}",
        ...
        with st.expander("✅ Checklist de Consistência (Koller)", expanded=False):
            ...
            checks = [
                ...
                (ss['gt'] <= 5.0, f"g terminal ({ss['gt']:.1f}%) ≤ PIB nominal de longo prazo (~5%)"),
                (ss['e_weight'] > 0.2, "Pesos de mercado usados (não contábeis)"),
                (net_debt == t_debt - cash_v, "Ativos não-operacionais (caixa) separados do FCF"),
                (cv_pct <= 80, f"CV = {cv_pct:.0f}% do EV — dentro do intervalo razoável (< 80%)"),
...
with tabs[7]:
    ...
    st.caption("Referência: Koller et al., Cap. 12 — *Using Multiples to Triangulate DCF*")

    st.info("Múltiplos são **contexto**, não substitutos do DCF. "

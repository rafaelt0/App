...
# Escolha modo de alocação
modo_manual = "Alocação Manual"
tipo_otimizacao = st.sidebar.selectbox(
    "Modo de alocação",
    [modo_manual, "Hierarchical Risk Parity (HRP)", "Max Sharpe Ratio", "Minimizar Volatilidade"]
)

if tipo_otimizacao == modo_manual:
    ...
    # Otimizações automáticas com PyPortfolioOpt
    if tipo_otimizacao == "Hierarchical Risk Parity (HRP)":
        hrp = HRPOpt(returns)
        ...
        if tipo_otimizacao == "Max Sharpe Ratio":
            weights = ef.max_sharpe()
        elif tipo_otimizacao == "Minimizar Volatilidade":
            weights = ef.min_volatility()
        else:
            st.error("Método de otimização inválido.")
            st.stop()
    peso_manual_df = pd.DataFrame.from_dict(weights, orient='index', columns=["Peso"])
...
# Aqui você pode seguir com o restante do seu código para análise, gráficos e métricas...
